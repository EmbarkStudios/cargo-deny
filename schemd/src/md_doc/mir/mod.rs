...
impl Context {
    ...
    fn schema_nested(&self, node: &SchemaNode, doc: &SchemaDocData) -> Document {
        let doc_name = doc
            .header
            .clone()
            .unwrap_or_else(|| node.schema.path_from_anchor().to_string());

        let nested = NamedDocument::new(doc_name, self.schema_embedded(node, doc));
    ...
    fn section_header(&self, schema: &Schema) -> String {
        if let Some(header) = schema.doc.data().header.clone() {
            return header;
        }

        if schema.path.segments.is_empty() {
        ...
        let header = schema.path_from_anchor().to_string();

        // if last_segment.is_index() {
        //     let begin = schema.path.segments[0..schema.path.segments.len() - 1]
        //         .iter()
        //         .rposition(|segment| !segment.is_index())
        //         .unwrap_or(0);

        //     let subpath = schema.path.segments[begin..].to_vec();
        //     let subpath = schema.path.clone().with_segments(subpath);
        //     return format!("`{subpath}`");
        // }

        let ty = schema.ty.inner.ty.as_deref();
        let array_items_ty = schema
            .ty
            .array_items_ty
            .as_ref()
            .and_then(|ty| ty.ty.as_deref());

        match (ty, array_items_ty) {
            (Some("object"), _) => format!("`[{header}]`"),
            (Some("array"), Some("object")) => format!("`[[{header}]]`"),
            _ => format!("`{header}`"),
        }
    }
        ...
        let reference = schema.doc.reference().map(|reference| {
            let (label, url) = reference
                ...
                .map(|definition| {
                    (
                        definition,
                        format!("/type-index/{}.md", definition.to_kebab_case()),
                    )
                })
    ...
    fn enum_doc(&self, ty: &LeafType) -> Option<Section> {
        let doc = ty
            ...
            .iter()
            .map(|enum_variant| {
                let value = format!("`{}`", enum_variant.value);
                let description = enum_variant.description();
    ...
    fn value(&self, key: &Path, value: &Value) -> String {
        ...
        let needs_wrapping = !value.is_object();
