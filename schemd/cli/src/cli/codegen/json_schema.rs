use crate::prelude::*;
use crate::serdex;
use crate::source::{RootSchema, Schema, Traverse};
...
impl<'a> GenContext<'a> {
    ...
    fn normalize_enum(&self, schema: &mut Schema) -> Result {
        ...
        let Some(enum_variants) = inlined.enum_variants() else {
            return Ok(());
        };

        let descriptions: Vec<Option<String>> = enum_variants
            .map(|variant| variant.x_schemd?.description)
            .collect();
