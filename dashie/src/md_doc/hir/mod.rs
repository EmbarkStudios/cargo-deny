...
pub(crate) struct LeafType {
    ...
    pub(crate) enum_schema: Option<Vec<dashie_schema::EnumVariantSchema>>,
}

impl LeafType {
    fn from_dashie_schema(schema: &dashie_schema::Schema) -> Self {
        Self {
            ty: schema.ty.clone(),
...
impl Type {
    pub(crate) fn from_source_schema(schema: &dashie_schema::Schema) -> Self {
        let inner = LeafType::from_dashie_schema(schema);

        let array_items_ty = schema
            .array_schema
            .as_ref()
            .map(|array| LeafType::from_dashie_schema(&array.items));
...
pub(crate) struct PathedSourceSchema {
    pub(crate) path: Path,
    pub(crate) inner: dashie_schema::Schema,
}

impl PathedSourceSchema {
    pub(crate) fn new(path: Path, inner: dashie_schema::Schema) -> Self {
        Self { path, inner }
    }

    pub(crate) fn origin(origin: PathOrigin, inner: dashie_schema::Schema) -> Self {
        Self::new(Path::new(origin), inner)
    }

    fn inline_referenced_definition(mut self, root: &dashie_schema::RootSchema) -> Result<Self> {
