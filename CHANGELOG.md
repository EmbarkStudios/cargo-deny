...
### Changed
- Updated `krates`, which in turn uses an updated `cargo_metadata` which uses [`camino`]() for utf-8 paths. Rather than support both vanilla Path/PathBuf and Utf8Path/PathBuf, cargo-deny now just uses Utf8Path/PathBuf, which means that non-utf-8 paths for things like you Cargo.toml manifest or license paths will no longer function. This is a breaking change, that can be reverted if it disruptive for users, but the assumption is that cargo-deny is operating on normal checkouts of rust repositories that are overwhelmingly going to be utf-8 compatible paths.
