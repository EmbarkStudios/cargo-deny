...
fn deserialize_table<'de, 'b>(
    ...
    let max = tables.len();
    if ctx.cur_parent == max || ctx.cur == max {
        return Ok(());
    }
    ...
    loop {
        ...
        if ctx.depth != ttable.header.len() {
            ...
            if dbg!(&table).contains_key(dbg!(&key)) {
            ...
            let value = if array {
                let mut arr = Vec::new();
                deserialize_array(ctx, tables, &mut arr)?;
                ValueInner::Array(arr)
            } else {
                let mut tab = super::Table::new();
                deserialize_table(ctx, tables, &mut tab)?;
        ...
        for (key, val) in ttable.values.take().expect("Unable to read table values") {
...
fn deserialize_array<'de, 'b>(
    ...
    while ctx.cur_parent < max {
        ...
        for (key, val) in tables[ctx.cur_parent]
            .values
            .take()
