...
mod test {
    ...
    fn search_match() {
        ...
        macro_rules! req {
            ($vr:expr) => {
                semver::VersionReq::parse($vr).unwrap()
            };
        }

        macro_rules! eq {
            ($ind:expr, $name:expr, $vs:expr) => {
                let ind = $ind.unwrap();
                assert!(
                    krates[ind].name == $name
                        && krates[ind].version == semver::Version::parse($vs).unwrap()
                );
            };
        }

        assert_eq!(search_match(&krates, "adler32", &req!("1.0")), Some(0));
        eq!(
            search_match(&krates, "tower-service", &req!("*")),
            "tower-service",
            "0.2.0"
        );

        assert_eq!(search_match(&krates, "tracing", &req!("0.0.9")), None);
        assert_eq!(search_match(&krates, "tracing", &req!("0.2")), None);
        eq!(
            search_match(&krates, "tokio-codec", &req!("0.2.0-alpha")),
            "tokio-codec",
            "0.2.0-alpha.6"
        );
        ...
        // Ensure that searching for a crate that doesn't exist, but would be sorted at the end
        // does not cause and out of bounds panic
        assert_eq!(search_match(&krates, "winit", &req!("*")), None);
    }
}
