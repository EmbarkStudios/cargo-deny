...
impl GraphContext {
    fn fill(cmd: Command) -> Command {
        cmd.args([
            ...
            Arg::new("CONFIG_PATH").long("config").help("Path to the config to use.").long_help("Path to the config to use.\n\nIf not specified, each directory from the one containing the manifest up to the root is searched for `deny.toml`, `.deny.toml`, `.cargo/deny.toml`, and `.config/deny.toml`, in that order.").value_parser(PathParser).value_hint(clap::ValueHint::FilePath),
            ...
            Arg::new("target").short('t').long("target").help("One or more platforms to filter crates by.").long_help("One or more platforms to filter crates by.\n\nIf a dependency is target specific, it will be ignored if it does not match 1 or more of the specified targets. This option overrides the top-level `targets = []` configuration value.").value_parser(TargetParser).value_name("CFG").action(clap::ArgAction::Append),
