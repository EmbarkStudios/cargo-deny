...
use super::source::{ArraySchema, ObjectSchema, OneOfVariantSchema};
...
struct DocOptions {
    root: source::RootSchema,
    max_nesting_in_file: usize,
}

pub(crate) struct Doc {
    root: SchemaNode,
    type_index: BTreeMap<String, SchemaNode>,
}
...
impl Doc {
    fn new(options: DocOptions) -> Result<Self> {
        // let all_schemas = || {
        ...
        let doc = GenerateDoc { options }.run()?;
...
impl GenerateDoc {
    fn run(self) -> Result<Doc> {
        let root = PathedSourceSchema::origin(PathOrigin::Root, self.options.root.schema.clone());
...
pub(crate) fn gen(root: &source::RootSchema) -> Result<()> {
