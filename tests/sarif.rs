use cargo_deny::{
    sarif::{SarifLog, Run, Tool, Driver, Rule, TextContent, DefaultConfiguration, Help, RuleProperties},
...
fn test_sarif_multiple_diagnostics() {
    ...
    assert!(results.iter().any(|r| r.message.text == "Vulnerability 1" && r.level == "error"));
    assert!(results.iter().any(|r| r.message.text == "Missing license" && r.level == "warning"));
    assert!(results.iter().any(|r| r.message.text == "Banned crate" && r.level == "error"));
}
...
fn test_sarif_empty_diagnostics() {
    ...
    let collector = SarifCollector::new();
    let sarif = collector.generate_sarif();
